...
//
// Move status='submitted' → 'published', stamp published_at. Org admin only.
...
export async function POST(
  ...
  if (current.status !== "submitted") {
    return NextResponse.json(
      { error: `Can only publish submitted testimonials (currently ${current.status}).` },
      { status: 409 },
    );
  }
  if (!current.quote?.trim()) {
    return NextResponse.json(
      { error: "Cannot publish a testimonial with no quote." },
      { status: 400 },
    );
  }
